...
export default function AuthScreen() {
  ...
  const submit = (provider: "email" | "google") => {
    if (provider === "email") {
      ...
      setTimeout(() => {
        if (mode === "signup") {
          if (!name.trim()) { setLoading(null); toast.error("Please enter your full name."); return; }
          if (password.length < 6) { setLoading(null); toast.error("Password must be at least 6 characters."); return; }
          const res = registerAccount(name, email, password);
          if (!res.ok) { setLoading(null); toast.error(res.error); return; }
          ...
          if (!acct) { setLoading(null); setPassword(""); toast.error("Invalid credentials or account does not exist."); return; }
    ...
    setLoading("google");
    setTimeout(() => { setUser({ name: "Google Roomie", email: "google@hostel.app" }); }, 1200);
  };

  const startDemo = () => {
    setLoading("demo");
    setTimeout(() => { enableDemoMode(); }, 600);
  };
  ...
  return (
    <div className="relative min-h-screen flex flex-col overflow-hidden">
      ...
      <div className="pointer-events-none absolute top-1/2 -right-32 h-80 w-80 rounded-full bg-electric-orange/30 blur-3xl animate-float-blob" style={{ animationDelay: "2s" }} />
      <div className="pointer-events-none absolute -bottom-32 left-1/3 h-80 w-80 rounded-full bg-neon-purple/25 blur-3xl animate-float-blob" style={{ animationDelay: "4s" }} />
      ...
      <div className="relative flex-1 flex items-center justify-center px-4 py-10">
        <div className="w-full max-w-md space-y-6">
          ...
          <div className="grid gap-3">
            {FEATURES.map(f => (
              <div key={f.title} className="glass rounded-2xl p-4 flex items-start gap-3 transition hover:bg-white/[0.08]">
          ...
          >
            {loading === "demo" ? <Loader2 className="h-5 w-5 animate-spin" /> : <Sparkles className="h-5 w-5" />}
            {loading === "demo" ? "Launching instant tour…" : "Try Instant Demo Tour"}
          ...
          <div className="glass-strong rounded-3xl p-6 sm:p-8">
            ...
            <div className="space-y-3">
              ...
              />
              {mode === "signup" && (
                <Field icon={<KeyRound className="h-4 w-4" />} placeholder="Room code / invite link (optional)" value={room} onChange={setRoom} />
              )}
